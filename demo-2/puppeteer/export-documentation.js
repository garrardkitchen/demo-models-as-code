...
(async () => {
  ...
  let route = decodeURI(url).split('://')[1].split('/')
  let name = route[route.length - 1].replace(' ','-')

  await page.exposeFunction('saveHtml', (content) => {
    const filename = FILENAME_SUFFIX + `${name}.html`;
    console.log(" - Writing " + filename);
    fs.writeFile(`documentation/${filename}`, content, 'utf8', function (err) {
      if (err) throw err;
    });
